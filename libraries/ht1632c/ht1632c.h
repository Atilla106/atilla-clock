...
class HT1632C {
public:
  HT1632C(uint8_t pinWR, uint8_t pinDATA, uint8_t pinCS[], uint8_t numDisplays);
...
private:
  ...
  void _chip_select(uint8_t n);
  void _chip_free(uint8_t n);

  void _write_bits(uint8_t bits, uint8_t firstBit);
  void _send_cmd(uint8_t disp, uint8_t command);
  void _send_data(uint8_t disp, uint8_t address, uint8_t data);
};
